...
module Frontend.Signe.Layout where
...
layoutWords = [( TokSymbol "let" 27
               , LayoutDelimiters (TokSymbol ";" 14) (Just (TokSymbol "{" 32)) (Just (TokSymbol "}" 33))
               )]

layoutStopWords :: [TokSymbol]
layoutStopWords = [TokSymbol "in" 26]
...
layoutSep   = List.nub $ TokSymbol ";" 14 : map (delimSep . snd) layoutWords

parenOpen, parenClose :: [TokSymbol]
parenOpen  = [TokSymbol "(" 2]
...
resolveLayout topLayout =
  ...
  topDelim = LayoutDelimiters (TokSymbol ";" 14) Nothing Nothing
